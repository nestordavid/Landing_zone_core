...
.NOTES
    File Name      : merge-tfvars-json.ps1
    Author         : William Robinson
    PS Version     : PowerShell 7.3
    Copyright 2023 - William Robinson
...
$MainKey = $TFVars.json.values.$MapKey
        ...
        foreach ($ResourceKey in $MainKey.keys) {
            ...
            if ($encounteredKeys.ContainsKey("$ResourceKey")) {
                Write-Error "Duplicate Resource found: $MapKey.$ResourceKey" -ErrorAction Continue
        ...
        elseif ($encounteredKeys["Duplicate"] -eq $true){
                Write-Error "Duplicates found - Stopping" -ErrorAction Stop
                exit
        }
